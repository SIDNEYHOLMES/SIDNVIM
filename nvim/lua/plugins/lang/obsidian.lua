return {
  ...
  opts = function()
    ...
    require 'telescope'
    local ok_conf, local_conf = pcall(require, 'config.local')
    local vault_base = vim.fn.expand((ok_conf and local_conf.notes) or '~/Documents/Notes/Obsidian')
    local workspaces = {}
    ...
    return {
      ...
      follow_url_func = function(url) require('config.open').open(url) end,
      attachments = {
